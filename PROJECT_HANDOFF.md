...
- Fourth homepage-body checkpoint: `41d9d28` (`feat: add responsive new arrivals story`). The obsolete Top Picks product rail is replaced by the shipped New Arrivals campaign story in the correct slot between Hot Deals and Trending.
- New Arrivals provides keyboard-accessible Men/Women/Kids tabs and 9 editable campaign blocks. Desktop renders two square banners plus one 16:5 banner; tablet and mobile render the three campaigns as full-bleed sticky 3:4 and 9:16 stories. The section includes image pickers with native focal-point support; repository images are temporary layout fallbacks and must be replaced by the 9 production campaign images.
- Targeted live QA passed at 390, 768, 1280 and 1920 px: tabs switch the correct panel by click and arrow keys, CTA sizes are 156/168/190 px by breakpoint, sticky positioning works after changing the main content guard from `overflow:hidden` to horizontal `overflow:clip`, and document-level horizontal overflow remains zero.
- Fifth homepage-body checkpoint: `ed45c06` (`feat: add responsive trending experience`). Trending now has keyboard-accessible Sneakers/Apparel/Accessories tabs, three editable campaign banners per category, and a native Dawn product rail per resolved collection. Banner navigation supports desktop arrows, dots at every breakpoint and touch scrolling on tablet/mobile; manual category selection resets that category's banner and product rail state.
- Trending live QA passed at 390, 768, 1280 and 1920 px: banners render at 16:9 on touch layouts and 16:5 on desktop; product rails render 2/3/5 cards with no document overflow; arrow, dot, touch, tab, keyboard and native Quick Add interactions passed. The only console noise came from Shopify's external Shop app iframe CSP/403 response, not homepage JavaScript.
- Catalog dependency: the public storefront currently returns 404 for `/collections/clothing`, so the Apparel product rail is intentionally empty until the catalog/import work publishes that collection. Keep the intended handle rather than substituting unrelated products. The current `new-arrivals` collection also includes at least one apparel item in the Sneakers tab; collection composition belongs to the catalog/import workstream.
- Sixth homepage-body checkpoint: `78c92c1` (`feat: build complete the look bundle flow`). The obsolete Flagship Spotlight and Clothing slots were replaced by one Complete the Look section after Shop by Brand. It provides 4 keyboard-accessible Look tabs, 2 editable gender banners per Look (8 production image slots), 5 editable product references per Look, desktop 74/24 banner-to-vertical-rail composition, stacked tablet rows and a two-card mobile rail.
- `SHOP LOOK` now opens an accessible Bundle Builder drawer. It requires a valid available variant for every configured product, submits every item in one Ajax cart request, adds a shared private `_Bundle group` identifier and `_Complete the Look` label to the line items, then refreshes and opens Dawn's native cart drawer. Focus trapping, Escape/overlay close, empty-cart transition and error states are implemented.
- Complete the Look live QA passed at 390, 768, 1280 and 1920 px with no document overflow or page JavaScript errors. A five-item live cart add produced five lines with one shared bundle ID; the test cart was cleared immediately afterward. The 20 current product references are active test defaults and should be curated by the catalog/import workstream; repository banner assets remain temporary fallbacks for the 8 final images.
