...
const source = JSON.parse(await fs.readFile(path.join(root,'data/footlocker/seed-groups.json'),'utf8'));
const out = path.join(root,'data/footlocker/details');
...
for (const group of source.groups) {
  for (const colorway of group.colorways) {
    ...
    try {
      ...
      await fs.writeFile(file,JSON.stringify({fetchedAt:new Date().toISOString(),sourceUrl:colorway.source_url,brand:group.brand,category:group.category,groupTitle:group.title,groupId:group.product_group_id,data,ld},null,2));
      count++;
...
await fs.writeFile(path.join(root,'data/footlocker/crawl-progress.json'),JSON.stringify({fetched:count,failures},null,2));
